...
const CreateAccountConfirmationComponent = (props) => {
  const [checked, setChecked] = useState(false);
  const renderIcon = () => {
    return <View style={{borderWidth: 2, borderColor: color.secondaryColor, borderRadius: 40, width: 38, height: 38, justifyContent: 'center', alignItems: 'center', marginTop: -6, marginRight: 16}}>
  ...
  const renderContent = () => {
    return <React.Fragment>
              <Text style={{fontSize: itemFontSize, lineHeight: getStyleOfDevice(34, 30)}}>
                សូមអានលក្ខខណ្ឌខាងក្រោម មុនពេលធ្វើការចុះឈ្មោះចូលប្រើប្រាស់កម្មវិធី សុខភាពយុវជន។ ដោយចុច <BoldLabelComponent label='"យល់ព្រម"' style={{fontSize: itemFontSize, lineHeight: getStyleOfDevice(34, 30)}}/> បញ្ចាក់ថាអ្នកយល់ព្រមទៅនឹងគោលការណ៍ឯកជនភាព និងលក្ខខណ្ឌនៃកម្មវិធី សុខភាពយុវជន។
              </Text>
              {renderCheckBox()}
  ...
  const renderCheckBox = () => {
    return <View style={{flexDirection: 'row', flexWrap: 'wrap', marginVertical: getStyleOfDevice(10, 4)}}>
              ...
              <Text style={{fontSize: itemFontSize, lineHeight: getStyleOfDevice(34, 30), marginTop: 8, flexWrap: 'wrap', flex: 1}}>
  ...
  return (
    <BottomSheetModalMainComponent
      ...
      titleStyle={{fontSize: bottomSheetTitleFontSize}}
...
export default CreateAccountConfirmationComponent;